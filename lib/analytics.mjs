...
// Each insight: { kind: 'alert'|'warn'|'good'|'neutral', category, headline, detail? }
// Sorted so the most attention-worthy rise to the top.
function buildInsights({
  ...
  if (training_load?.zone) {
    ...
    if (z === 'DANGER') {
      out.push({
        kind: 'alert',
        category: 'LOAD',
        headline: `Training load in danger zone — high injury/illness risk.`,
        detail: `acwr ${tl.acwr} · acute 7d ${tl.acute_7d_strain} vs chronic 28d ${tl.chronic_28d_strain} · dial back.`,
      });
      ...
      out.push({
        ...
        detail: `sustainable short-term; watch recovery closely.`,
      });
  ...
  // --- HRV vs 60d baseline ---
  if (recovery?.hrv_vs_baseline_pct != null && recovery.hrv_current_7d_ms && recovery.hrv_baseline_60d_ms) {
    const d = recovery.hrv_vs_baseline_pct;
    if (d >= 5) {
      out.push({
        kind: 'good',
        category: 'HRV',
        headline: `HRV +${d}% above your 60-day baseline.`,
        detail: `7d avg ${recovery.hrv_current_7d_ms}ms · baseline ${recovery.hrv_baseline_60d_ms}ms.`,
      });
    } else if (d <= -5) {
      out.push({
        kind: 'warn',
        category: 'HRV',
        headline: `HRV ${Math.abs(d)}% below baseline — signs of accumulated strain.`,
        detail: `7d avg ${recovery.hrv_current_7d_ms}ms · baseline ${recovery.hrv_baseline_60d_ms}ms.`,
      });
    }
  }
