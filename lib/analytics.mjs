...
function compute({ profile, body, cycles, recoveries, sleeps, workouts }) {
  ...
  const vitalTrends = buildVitalTrends(recoveries, sleeps);
  ...
  const deloadSignal = buildDeloadSignal({ training_load: trainingLoad, recoveries });
  const insights = buildInsights({
    ...
    periodicity, deload_signal: deloadSignal,
  });
...
// ---- vital trends: spo2, skin temp, respiratory rate over time ----
// Respiratory rate lives on sleep records (not recovery records). Elevated RR
// alongside skin-temp rise and SpO₂ drop is a well-documented pre-illness signal.
function buildVitalTrends(recoveries, sleeps) {
  const scoredRecs = recoveries.filter(scored);
  const last30 = scoredRecs.slice(0, 30);
  ...
  // Respiratory rate from sleep data.
  const scoredNights = sleeps.filter((s) => !s.nap && scored(s));
  const last30Nights = scoredNights.slice(0, 30);
  const rrVals = last30Nights.map((s) => s.score.respiratory_rate).filter((v) => v != null);
  const rrMean = mean(rrVals);
  const rrSd = stddev(rrVals);
  const latestRR = scoredNights[0]?.score?.respiratory_rate;
  const rrFlag = latestRR != null && rrSd
    ? Math.abs(latestRR - rrMean) > rrSd
    : false;
  // Direction matters for RR: only elevated RR is a pre-illness signal; low RR is not.
  const rrDirection = latestRR != null && rrMean != null
    ? (latestRR > rrMean ? 'UP' : latestRR < rrMean ? 'DOWN' : 'FLAT')
    : null;
  ...
  return {
    ...
    respiratory_rate: {
      avg_30d: round(rrMean, 2),
      stddev_30d: round(rrSd, 2),
      latest: latestRR != null ? round(latestRR, 2) : null,
      deviation_flag: rrFlag,
...
// ---- deload detector: high load + falling recovery together ----
// A deload is warranted when training stress has been high for a sustained
// period AND recovery is trending down. Either signal alone is ambiguous
// (high load with steady recovery = adaptation; falling recovery with normal
// load = external stressor). Together they're consensus.
function buildDeloadSignal({ training_load, recoveries }) {
  const window = training_load?.series?.slice(-14) || [];
  if (window.length < 10) return null;

  const highLoadDays = window.filter((p) => p.acwr != null && p.acwr > 1.3).length;
  const highLoadPct = highLoadDays / window.length;

  const scoredRecs = recoveries.filter(scored).slice(0, 14);
  if (scoredRecs.length < 10) return null;
  const recoScores = [...scoredRecs].reverse().map((r) => r.score.recovery_score);
  const recoSlope = linearSlope(recoScores);

  // ≥50% of the window at elevated load AND recovery trending down ≥0.3pt/day.
  const deloadWarranted = highLoadPct >= 0.5 && recoSlope != null && recoSlope < -0.3;

  return {
    window_days: window.length,
    high_load_days: highLoadDays,
    high_load_pct: round(highLoadPct * 100, 0),
    recovery_slope: round(recoSlope, 3),
    deload_warranted: deloadWarranted,
  };
}
...
function buildInsights({
  ...
  sport_impact, forecast, bedtime_drift, periodicity, deload_signal,
}) {
  const out = [];

  // --- Deload signal (highest priority after illness) ---
  if (deload_signal?.deload_warranted) {
    out.push({
      kind: 'alert',
      category: 'DELOAD',
      headline: `Deload week warranted — ${deload_signal.high_load_days}/${deload_signal.window_days} days at elevated load, recovery trending ${deload_signal.recovery_slope}pt/day.`,
      detail: `both load and fatigue signals agree — stronger than either alone.`,
      action: `cut training volume 40% for 7 days · keep easy aerobic + sleep.`,
    });
  }
  ...
  // --- Vital deviations (pre-illness signal) ---
  // RR elevation matters; RR dropping is not a pre-illness signal, so gate on direction.
  const rrElevated = vital_trends?.respiratory_rate?.deviation_flag
    && vital_trends.respiratory_rate.direction === 'UP';
  const markers = [];
  if (vital_trends?.skin_temp?.deviation_flag) markers.push('skin temp');
  if (vital_trends?.spo2?.deviation_flag) markers.push('SpO₂');
  if (rrElevated) markers.push('respiratory rate');

  if (markers.length >= 2) {
    // Composite warning — much higher signal than any single marker.
    out.push({
      kind: 'alert',
      category: 'ILLNESS',
      headline: `${markers.length} of 3 pre-illness markers elevated: ${markers.join(', ')}.`,
      detail: `elevated skin temp + respiratory rate + altered SpO₂ commonly precede symptomatic illness by 24–48h.`,
      action: `treat today as a rest day · hydrate · add 1h sleep tonight.`,
    });
  } else if (markers.length === 1) {
    // Single-marker alerts retain their original framing.
    ...
    if (rrElevated) {
      out.push({
        kind: 'alert',
        category: 'VITALS',
        headline: `Respiratory rate elevated — ${vital_trends.respiratory_rate.latest} vs 30d avg ${vital_trends.respiratory_rate.avg_30d} rpm.`,
        detail: `elevated RR alone often precedes illness by 24–48h.`,
        action: 'prioritize sleep · defer high-intensity; re-check in the morning.',
      });
    }
  }
  ...
  if (personal_records?.last_30d && timeseries?.length) {
    ...
    if (rec && recentDates.has(rec)) {
      out.push({
        kind: 'good',
        category: 'MILESTONE',
        headline: `New 30d recovery high of ${round(p.highest_recovery.val, 0)}% this week.`,
        detail: `on ${rec}.`,
        action: null,
      });
      ...
      out.push({
        kind: 'good',
        category: 'MILESTONE',
        headline: `New 30d HRV high of ${round(p.highest_hrv.val, 1)}ms this week.`,
        detail: `on ${hrv}.`,
        action: null,
      });
    }
